...
import org.eclipse.dataspaceconnector.common.statemachine.EntitiesProcessor;
...
public class ProviderContractNegotiationManagerImpl implements ProviderContractNegotiationManager {
    ...
    public void start(ContractNegotiationStore negotiationStore) {
        ...
        loop = StateMachineLoop.Builder.newInstance("provider-contract-negotiation", monitor, waitStrategy)
    ...
    private EntitiesProcessor<ContractNegotiation> processNegotiationsInState(ContractNegotiationStates state, Function<ContractNegotiation, Boolean> function) {
        return new EntitiesProcessor<>(() -> negotiationStore.nextForState(state.code(), batchSize), telemetry.contextPropagationMiddleware(function));
    }

    private EntitiesProcessor<ContractNegotiationCommand> onCommands(Function<ContractNegotiationCommand, Boolean> process) {
        return new EntitiesProcessor<>(() -> commandQueue.dequeue(5), process);
    }
